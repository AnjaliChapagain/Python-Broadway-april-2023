...
# Sets are unordered datatype i.e. {1, 2} and {2, 1} are same.
...
s = {[1, 2, 3, 4]} # This is not possible in set because all the elements must be immutable in set.
# But, it has a list.
...
s.update([5, 6, 7]) # We can use update() method to update a set.
# Update method takes an iterable / sequence as a parameter
...
# The difference in discard() and remove() method is, remove() raises an error if the value
# to be removed is not present in the set. But, the discard() method returns None if the value
# to be removed is not present in the set.
